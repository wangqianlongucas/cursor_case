...
class UnifiedTSPEnvironment:
    def __init__(self, num_cities=10, seed=None, state_mode='enhanced'):
        """
        Unified TSP environment supporting both simple and enhanced state representations
        
        Args:
            num_cities (int): Number of cities in the TSP
            seed (int): Random seed for reproducibility
            state_mode (str): 'simple' for basic Q-learning, 'enhanced' for DQN/PPO
        """
        ...
        # Precompute features for enhanced mode
        if self.state_mode == 'enhanced':
    ...
    def _get_simple_state(self):
        """Get simple state representation for Q-learning."""
    ...
    def _get_enhanced_state(self):
        """Get enhanced state representation for DQN/PPO."""
        state_features = []
        ...
        return torch.FloatTensor(state_features) if torch.is_tensor(torch.tensor(0)) else np.array(state_features)
    ...
    def step(self, action):
        ...
        if action in self.visited_cities:
            penalty = -100 if self.state_mode == 'simple' else -50
            return self._get_state(), penalty, True, {"valid": False}
        ...
        if self.state_mode == 'simple':
            reward = self._calculate_simple_reward(distance, done)
        else:
            reward = self._calculate_enhanced_reward(distance, done, prev_city, action)
        
        return self._get_state(), reward, done, {"valid": True}
    
    def _calculate_simple_reward(self, distance, done):
        """Calculate simple reward for Q-learning."""
        if done:
            # Add return distance to starting city
            return_distance = self.distance_matrix[self.current_city][0]
    ...
    def _calculate_enhanced_reward(self, distance, done, prev_city, current_city):
        """Calculate enhanced reward for DQN/PPO."""
        reward = 0
    ...
    def get_action_mask(self):
        """Get action mask for PPO (1 for valid actions, 0 for invalid)."""
        mask = np.zeros(self.num_cities)
        for action in self.get_valid_actions():
            mask[action] = 1
        return mask
    
    def render(self, title_suffix=""):
        ...
        for i in range(len(self.visited_cities) - 1):
            ...
            # Add direction arrows for enhanced mode
            if self.state_mode == 'enhanced':
        ...
        title = f'TSP Tour - {self.state_mode.title()} Mode (Distance: {self.total_distance:.2f}, Cities: {len(self.visited_cities)}/{self.num_cities})'
        if title_suffix:
